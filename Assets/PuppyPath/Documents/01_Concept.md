...
7. 当用户点击右上方地图按钮并打开大地图后，可以点击狗爪 marker 选择目标景点；系统显示场地道路和本次路线，然后进入导航模式。
...
- 地图按钮打开居中的完整大地图。
- 狗爪 marker 代表各个景点。
- 地图 UI 显示 `VenueMapDefinition.navGraph` 中已经设定好的道路网络。
- 用户点击某个狗爪 marker 后，系统从用户当前位置到目标景点生成本次路线，并在地图 UI 和地面同时显示路线；如果用户不点 marker，则仍然保持自由行走状态。
- 用户不再通过点击地图任意位置来生成 marker 或选择目的地。
- 关闭或取消大地图时返回自由行走模式。
- `FriendListPanel`、`MapPanel` 和 `XPanel` 作为同一组主界面一起显示 / 隐藏；旧 `IntroPanel` 不再显示。
- `NavigationHudPanel` 只在主界面组隐藏后显示。

### 朋友列表区域

- 当前版本取消朋友列表导航。
- 原本朋友列表所在区域只保留为流程文案 / intro 文案展示区域。
- 用户不能通过朋友按钮进入导航，也不再使用旧版 `Path_Kevin`、`Path_Ying` 等 waypoint prefab。
- 当前可导航目标只来自大地图上的景点 marker。

### 导航模式

- 地面出现一条从用户当前位置附近通往目标景点的 line。
- 小狗在前面带路。
- 顶部 UI 文案：`Paws this way to {景点名}!`
- 导航模式 UI 必须提供一个明确的退出 / 取消导航按钮，例如：`退出导航`。
- 退出 / 取消导航按钮只在导航模式显示；自由探索时不显示。
- 用户点击退出导航后，系统清除地面路线，取消当前目标，小狗回到自由行走跟随状态。
- 小狗仍然必须在可行走区域内，并保持在用户前方可见位置。
- 当用户进入景点范围后，系统完成当前导航，播放到达反馈，然后 `NavigationHudPanel` 回到随便逛逛文案。
